...
const Feed = (props) => {
  ...
  const { title, filter, data, meta, loading } = useAppSelector(
        ...
        <div className="content__body">
          <Grid grid-template-columns="50px 90px 200px 180px 200px 270px 170px 220px 220px">
            <div className="head">
                ...
                <Field>
                  <Checkbox
                    ...
                    onSelect={(status, context) => {
                      const ids =
                        context.entity.getAttribute("index_records_ids");

                      for (const id in ids) {
                        context.setAttribute(
                          `index_records_ids.${id}`,
                          status ? 1 : 0,
                        );
                      }
              ...
              <div>
                Номер/Скан счёта
                <sup className="grid_list__item sup">
                  Number/Bills scan copy
                </sup>
              </div>
              <div>
                Статус/Дата оплаты
              ...
              <div>
                ...
                <sup className="grid_list__item sup">
                  Gegenstand der Rechnung/Anmerkungen
                </sup>
              </div>
              <div>
                Назначение платежа
            ...
            {data.map((record, index) => {
              ...
              const { bill, status, client, object, contractor } =
                record.relationships;

              const data_json = recordData.data_json
                ? JSON.parse(recordData.data_json).info
                : [];

              return (
                <div
                  linkTo={`/payments/view/${recordData.id}`}
                  key={recordData.id}
                >
                  <div noRoute>
                    <Field>
                    ...
                    <Field>
                      <Checkbox
                        ...
                        onSelect={async (status) => ({
                          status: await Payments.setAccept(
                            recordData.id,
                            status,
                          ),
                        })}
                  ...
                  <div noRoute>
                    {bill && bill.attributes && (
                      <span>
                        <b>{bill.attributes.number}</b>
                        <br></br>от{" "}
                        {moment(bill.attributes.date).format("DD.MM.Y")}
                      </span>
                    )}
                  ...
                  <div>
                    {status && status.attributes && (
                      <span>
                        <b>{status.attributes.name}</b>
                        <br></br>
                        <i>
                          план{" "}
                          {moment(recordData.date_payment).format("DD.MM.Y")}
                        </i>{" "}
                        ({recordData.pay_week})
                      </span>
                    )}
                  ...
                  <div>
                    <b>
                      {object && object.attributes && object.attributes.name}
                    </b>
                    <br></br>
                  ...
                  <div>
                    <b>
                      {client && client.attributes && client.attributes.name}
                    </b>
                    <br></br>
                    {contractor &&
                      contractor.attributes &&
                      contractor.attributes.name}
                  </div>
                  <div>
                    <b>
                      {recordData.summa} <u>{recordData.currency}</u>
                    </b>
                    <br></br>
                    <i>
                      {recordData.pay_type} ({recordData.percent_of_bill}%)
                    </i>
                  </div>
                  <div>
                    {bill && bill.attributes.comment}
                    <br></br>
