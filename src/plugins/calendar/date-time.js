...
class DateTime {
  ...
  getWeeksList() {
    ...
    const weeks = [];
    let weekNumber = 1;
    const monday = this.getFirstMondayOfYear();

    while (i < 365) {
      weeks[weekNumber] = `${weekNumber}  (${monday.getDate()} ${
        this.monthsList[monday.getMonth()][0]
      }`;
      monday.setDate(monday.getDate() + 6);
      weeks[weekNumber] += ` - ${monday.getDate()} ${
        this.monthsList[monday.getMonth()][0]
      })`;
  ...
  getFirstMondayOfYear() {
    ...
    const date4Jan = new Date(new Date().getFullYear(), 0, 4);
    const date4JanDayOfWeek = date4Jan.getDay();
    const delta =
      date4Jan.getDate() -
      date4JanDayOfWeek +
      (date4JanDayOfWeek == 0 ? -6 : 1); // adjust when day is sunday
  ...
  buildList() {
    ...
    for (let i = 0; i < tasks.data.length; i++) {
      const target = document.getElementById("targetItem").cloneNode(true);
      target.id = `item${i}`;
  ...
  setDateMask(params) {
    ...
    return params["mask"]
      ...
      .replace(
        /MMMM/g,
        this.monthsList[date.getMonth()][params["declension"] ? 2 : 1],
      )
